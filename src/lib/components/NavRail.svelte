<script lang="ts">
  ...
  // Bağlantı / Performans / Günlük artık Ayarlar içinden açılır (rail sadeleştirildi).
  const items = [
    ...
    // "Hız Sınırı" (limits) GİZLENDİ — indirme throttle WinDivert inbound capture istiyor; taze BSOD
    // (DPC_WATCHDOG) sonrası kernel yükü eklememek için kapatıldı. Upload-QoS güvenli ama özellik
    // bütünüyle canlı test edilene dek gizli (real.rs inbound kodu uykuda: hiçbir yerden tetiklenmiyor).
